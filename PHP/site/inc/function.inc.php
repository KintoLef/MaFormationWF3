...
{
    if(utilisateur_connecte() && $_SESSION['utilisateur']['statut'] == 1)
    {
        return true;
    }
    return false;
...
function retirer_article_du_panier($id_article)
{
    ...
    if($position !== FALSE)
    {
        ...
        // array_splice() permet de supprimer un élément dans un tableau et surtout de réordonner les indices afin de ne pas avoir de trous dans notre tableau.
        // array_splice(tableau_concerné, indice_à_supprimer, nb_élément_à_supprimer)
    }
}
...
function montant_total()
{
    ...
    {
        $taille_table = sizeof($_SESSION['panier']['id_article']);
        $total = 0;
